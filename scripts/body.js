...
document.addEventListener("DOMContentLoaded", () => {
    ...
    celFehLinks.forEach((link) => {
        ...
        link.addEventListener("click", function (event) {
            ...
            // Check if the clicked link has the active class (assuming it represents Fahrenheit)
            if ( /* link.classList.contains("active") &&  */ link.id === "feh") {
                if (!isConverted) {
                    // Convert Celsius temperatures to Fahrenheit for weather cards
                    const cardWeathersCelsius = document.querySelectorAll(".card-weather");
                    const updatedCardTemps = [];
                    
                    //for each of the card weathers
                    cardWeathersCelsius.forEach((weatherElement) => {
                        //parse the celsius to float
                        const celsius = parseFloat(weatherElement.textContent);
                        // Convert without rounding
                        const fahrenheit = (celsius * 9) / 5 + 32;
                        //put in the updated card weathers array in the following format
                        updatedCardTemps.push(`${fahrenheit.toFixed(1)}°F`);
                    });
                    
                    // Convert Celsius temperature to Fahrenheit for the big "degree" element on the left
                    // parse the celsius degree to float
                    const celsiusDegree = parseFloat(degreeElement.textContent);
                    // Convert without rounding
                    const fahrenheitDegree = (celsiusDegree * 9) / 5 + 32;
                    
                    // Update all elements simultaneously after conversion
                    cardWeathersCelsius.forEach((weatherElement, index) => {
                        weatherElement.textContent = updatedCardTemps[index];
                    });
                    degreeElement.textContent = `${fahrenheitDegree.toFixed(1)}°F`;
                    
                    // Mark as converted
                    isConverted = true;
                }
            }
            //if its the celsius link, revert to Celsius
            else {
                // check if isConverted is true, which is true if the fehrenheit is clicked before
                if (isConverted) {
                    // Convert Fahrenheit temperatures back to Celsius for weather cards
                    const cardWeathersFahrenheit = document.querySelectorAll(".card-weather");
                    const updatedCardTemps = [];
                    
                    //for each of the cards
                    cardWeathersFahrenheit.forEach((weatherElement) => {
                        
                        //parse the fehrenheit degree to float
                        const fahrenheit = parseFloat(weatherElement.textContent);
                        // Convert without rounding
                        const celsius = ((fahrenheit - 32) * 5) / 9;
                        //push in the updated cards array
                        updatedCardTemps.push(`${celsius.toFixed(1)}°C`);
                    });
                    
                    // same for the big "degree" element on the left
                    const fahrenheitDegree = parseFloat(degreeElement.textContent);
                    const celsiusDegree = ((fahrenheitDegree - 32) * 5) / 9; // Convert without rounding
                    
                    // Update all elements simultaneously after conversion
                    cardWeathersFahrenheit.forEach((weatherElement, index) => {
                        weatherElement.textContent = updatedCardTemps[index];
                    });
                    degreeElement.textContent = `${celsiusDegree.toFixed(1)}°C`;
